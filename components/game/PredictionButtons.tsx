      ...
      <div className="grid grid-cols-5 gap-1.5 sm:gap-2 w-full">
        {numberPredictions.map((num) => (
          <button
            ...
            className={`relative bg-red-600 text-white font-bold py-3 sm:py-4 px-1 sm:px-2 rounded-xl hover:bg-red-700 transition-all shadow-lg hover:shadow-xl text-sm sm:text-base min-w-0 ${
              selectedPrediction === num ? "ring-2 sm:ring-4 ring-yellow-400 scale-105" : ""
            }`}
          >
            {num === "3" && (
              <div className="absolute -top-1 -right-1 sm:-top-2 sm:-right-2 w-6 h-6 sm:w-8 sm:h-8 bg-green-500 rounded-full flex items-center justify-center text-xs font-bold border-2 border-white shadow-md">
                3
              </div>
            )}
            {num}
        ...
        {colorPredictions.map((pred) => (
          <button
            ...
            className={`bg-red-600 text-white font-bold py-3 sm:py-4 px-2 sm:px-4 rounded-xl hover:bg-red-700 transition-all shadow-lg hover:shadow-xl text-xs sm:text-sm min-w-0 ${
              selectedPrediction === pred.value ? "ring-2 sm:ring-4 ring-yellow-400 scale-105" : ""
            }`}
          >
